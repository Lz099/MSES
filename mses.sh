...
KVER=`uname -a`
#variable pour savoir si Homebrew doit être installé
MSFPASS=`openssl rand -base64 16`
#variable avec l'heure de lancement utilisée pour les logs
...
{
	if [ "$(id -u)" != "0" ]; then
	   print_error "This step must be executed as root"
	   exit 1
	fi
}
########################################
...
function install_armitage_osx
{
	if [ -e /usr/bin/curl ]; then
		print_status "Downloading the latest version of Armitage"
		curl -# -o /tmp/armitage.tgz http://www.fastandeasyhacking.com/download/armitage-latest.tgz && print_good "Finished"
		if [ $? -eq 1 ] ; then
		       print_error "Failed to download latest version of Armitage"
		else
			print_status "Unpacking the package into /opt/armitage"
			tar -xvzf /tmp/armitage.tgz -C /usr/local/
	    fi

	   #Vérifiez si les liens existent et s'ils ne les créent pas
	    if [ ! -e /usr/local/bin/armitage ]; then
	    	print_status "Link Armitage in /usr/local/bin/armitage"
	    ...
	    if [ ! -e /usr/local/bin/teamserver ]; then
	    	print_status "Copying Teamserver to  /usr/local/bin/teamserver"
...
function usage ()
{
	...
	echo "-g                :Install GNU GCC (Not necessary uless you wish to compile and install ruby OSX"
